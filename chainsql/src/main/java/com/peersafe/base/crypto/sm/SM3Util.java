...
public class SM3Util extends GMBaseUtil {
     ...
     */
    public static byte[] prefixedHash(byte[] prefixed, byte[] srcData) {

        byte[] resArray = new byte[prefixed.length + srcData.length];
        ByteBuffer buff = ByteBuffer.wrap(resArray);
        buff.put(prefixed);
        buff.put(srcData);
